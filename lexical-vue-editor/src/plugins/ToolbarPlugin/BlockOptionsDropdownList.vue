...
onMounted(() => {
  if (props.toolbarRef && dropDownRef.value) document.addEventListener('click', handle);
});
...
function formatLargeHeading() {
  if (props.blockType !== 'h1') {
    editor.update(() => {
      ...
      if ($isRangeSelection(selection)) $wrapNodes(selection, () => $createHeadingNode('h1'));
    });
  }
  emit('update:showBlockOptionsDropDown', false);
}
...
function formatSmallHeading() {
  if (props.blockType !== 'h2') {
    editor.update(() => {
      ...
      if ($isRangeSelection(selection)) $wrapNodes(selection, () => $createHeadingNode('h2'));
    });
  }
  emit('update:showBlockOptionsDropDown', false);
}
...
function formatBulletList() {
  if (props.blockType !== 'ul') editor.dispatchCommand(INSERT_UNORDERED_LIST_COMMAND, undefined);
...
function formatNumberedList() {
  if (props.blockType !== 'ol') editor.dispatchCommand(INSERT_ORDERED_LIST_COMMAND, undefined);
  else editor.dispatchCommand(REMOVE_LIST_COMMAND, undefined);
...
function formatQuote() {
  if (props.blockType !== 'quote') {
    editor.update(() => {
      ...
      if ($isRangeSelection(selection)) $wrapNodes(selection, () => $createQuoteNode());
    });
  }
  emit('update:showBlockOptionsDropDown', false);
}
