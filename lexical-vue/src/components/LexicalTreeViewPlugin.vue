...
function $getSelectionStartEnd(node: LexicalNode, selection: BaseSelection): [number, number] {
  ...
  if ($isNodeSelection(selection) || anchorAndFocus === null) return [-1, -1];
  ...
  if (isPlaying.value) {
    const play = () => {
      ...
      timeoutId = setTimeout(() => {
        ...
        if (input !== null) input.value = String(index);
...
<template>
  <div :class="viewClassName">
    ...
    <div v-if="timeTravelEnabled && (showLimited || !isLimited)" :class="timeTravelPanelClassName">
      ...
      />
      <button :class="timeTravelPanelButtonClassName" @click="exit">Exit</button>
    </div>
  </div>
</template>
